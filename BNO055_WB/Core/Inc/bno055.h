...
//Address of IMU (change the input to COM3 Pin)
#define BNO055_I2C_ADDR_HI			0x29<<1  //COM3 HIGH
#define BNO055_I2C_ADDR_LO			0x28<<1  //COM3 LOW
#define CHIP_ID		0xA0
#define ACC_ID		0xFB
#define MAG_ID		0x32
#define GYR_ID		0x0F
...
//Initialization
uint8_t BNO_INIT( BNO *dev,I2C_HandleTypeDef *i2cHandel); //will return 0 if no errors
...
HAL_StatusTypeDef BNO_ReadRegisters(BNO *dev,uint8_t reg, uint8_t *data, uint8_t length); //read from multiple registers so save mu;tiple bytes

HAL_StatusTypeDef BNO_WriteRegister(BNO *dev,uint8_t reg, uint8_t *data); //write to a register
HAL_StatusTypeDef BNO_WriteRegisters(BNO *dev,uint8_t reg, uint8_t *data, uint8_t length); //write to a register

HAL_StatusTypeDef BNO_SelectBank(BNO *dev, uint8_t *data); //select bank before W/R registers
