...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_PpsHdle;                    /**< my_p2p_server handle */
  uint16_t  CustomLed_CHdle;                  /**< LED_C handle */
  uint16_t  CustomSwitch_CHdle;                  /**< SWITCH_C handle */
  uint16_t  CustomMy_MouseHdle;                    /**< my_mouse handle */
  uint16_t  CustomMouseHdle;                  /**< mouseData handle */
...
#define COPY_MOUSEDATA_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x03,0xc3,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomMouseHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MOUSE_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MOUSE_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomMouseHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_PpsHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: MY_PPS, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: MY_PPS \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomMy_PpsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeLed_C,
   ...
   * service_max_attribute_record = 1 for my_mouse +
   *                                2 for mouseData +
   *                                1 for mouseData configuration descriptor +
   *                              = 4
   *
   * This value doesn't take into account number of descriptors manually added
   * In case of descriptors added, please update the max_attr_record value accordingly in the next SVCCTL_InitService User Section
  ...
  ret = aci_gatt_add_service(UUID_TYPE_16,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_MouseHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: my_mouse, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: my_mouse \n\r");
  }

  /**
   *  mouseData
   */
  COPY_MOUSEDATA_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMy_MouseHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMouse,
                          CHAR_PROP_NOTIFY,
                          ATTR_PERMISSION_NONE,
                          ...
                          &(CustomContext.CustomMouseHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MOUSE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MOUSE \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_PpsHdle,
                                       CustomContext.CustomLed_CHdle,
                                       0, /* charValOffset */
    ...
    case CUSTOM_STM_MOUSE:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_MouseHdle,
                                       CustomContext.CustomMouseHdle,
                                       0, /* charValOffset */
                                       SizeMouse, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MOUSE command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MOUSE command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_C:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_PpsHdle, CustomContext.CustomLed_CHdle, SizeLed_C, pPayload);
    ...
    case CUSTOM_STM_SWITCH_C:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_PpsHdle, CustomContext.CustomSwitch_CHdle, SizeSwitch_C, pPayload);
    ...
    case CUSTOM_STM_MOUSE:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_MouseHdle, CustomContext.CustomMouseHdle, SizeMouse, pPayload);
